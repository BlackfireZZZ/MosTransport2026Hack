...
`stops` — физические остановки, `network_edges` — рёбра перемещения/пересадки, `route_stops` — прохождение маршрута по графу. `forecast_points` хранит точки версионированного прогноза. Ключ: `(run_id, route_id, stop_id, direction_id, bucket_start)`; горизонт закреплён за запуском. [ADR-0006](../decisions/0006-stop-identity-and-direction.md) сохраняет направление отдельной осью физической остановки. Колонка и уникальность введены новой миграцией TASK-027. Границы интервала и вместимость могут быть неизвестны (`null`).
