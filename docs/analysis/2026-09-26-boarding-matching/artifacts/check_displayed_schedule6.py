...
for w in data:
 ...
 for method in ['consensus3_5s','adaptive_scan','feature_gmm']:
  gaps=np.diff(w['methods'][method]);fit=align(gaps,edges,mask,scales=(1.,),max_step=6);direct=align(gaps,edges,mask,max_step=1,scales=(1.,));flex=align(gaps,edges,mask)
  rng=np.random.default_rng(20260926);null=[align(gaps,rng.permutation(edges),mask,scales=(1.,),max_step=6)['mae_seconds'] for _ in range(9)]
  intervals=[]
...
(folder/'displayed-window-schedule-check-6steps.json').write_text(json.dumps(results,ensure_ascii=False,indent=2))
