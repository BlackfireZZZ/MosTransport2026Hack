...
| TASK-039 | [Build a first-data profiling and adaptation toolkit](#task-039) | feature | data,ml | P1 | R2 | done | intake-profiling lead | DATA | TASK-017, TASK-018, TASK-019 | E | M |
...
- **Correction (2026-09-25):** the `*_units` columns added during this task's review were not clipped at the cutoff, so a coverage hole lying entirely after the cutoff moved the model-visible digest. Found by the TASK-020 review and fixed there in `6f28a04`, with a coverage-perturbation case added to `test_features_leakage.py`; the `year/month` digest superseded `408d4580…` with `58e57f49…`, `day/hour` and `month/day` are unchanged, and all three are now pinned by a test.
...
- **Execution:** `agent/rolling-backtest`, base `c34e7d9`, merged into `main` as `9a3cf55`; [scope, fold rules and review record](../exec-plans/completed/rolling-backtest.md).
- **Verification (2026-09-25):** independent acceptance validation ACCEPT — test windows enumerated pairwise over a ten-year coverage with zero overlaps and zero ordering violations (day 3645 folds, month 107, year 6), a direct partition audit found no row reaching a model with `event_at >= cutoff` or `available_at > cutoff`, and every fold count, origin range and hash family reproduced. Independent review ACCEPT WITH FINDINGS, all fixed in `0442c92`: `insufficient_train_history` counted calendar rather than covered buckets, so a fold standing on 1 covered day of an 821-bucket window reported success; the shared `FoldData` was mutable, so one model could corrupt the next model's input; and six published `manifest_hash` values did not reproduce because the model set was never recorded. Both reviewers independently endorsed the hour-aligned day period and the zero embargo. `make check`: exit 0 — backend 292 passed / 10 SQL skipped, ML 479 passed, frontend 47 passed, reference contracts 119 passed.
...
- **Execution:** `agent/intake-profiling`, base `c34e7d9`, merged into `main` as `cd55f2a`; [scope, privacy rule and review record](../exec-plans/completed/intake-profiling.md).
- **Verification (2026-09-25):** independent acceptance validation ACCEPT — all four criteria reproduced, including the report digest rebuilt from a fixture the validator generated itself, and a third source schema of its own that reconciled while five deliberate perturbations each moved exactly the right counts. Independent review REJECTED the first submission: a headerless CSV printed row one's cells as column names, putting a card-shaped value in the refusal four times and in the report file, bypassing identifier suppression because the value arrived labelled as schema. Fixed in `0eb2349`/`a12d0b3`/`2309c20`, then the lead's own probe showed the per-name `isidentifier` gate was calibrated to the reported cells rather than the class and still leaked a letter-prefixed card number; closed in `c1c4dfd` by judging the whole column line instead — a header is trusted only when every cell is word-like. Residual hole disclosed and recorded as TD-003. `make check`: exit 0 — backend 292 passed / 10 SQL skipped, ML 332 passed, frontend 47 passed, reference contracts 119 passed.
